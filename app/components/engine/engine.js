var Game = {
  ...
  create: function () {
    ...
    this.add.text(30, 20, 'SCORE:', {font: 'bold 16px sans-serif', fill: '#46c0f9', align: 'center'});
    this.scoreText = this.add.text(100, 20, score.toString(), {
  ...
  checkWallCollision: function (head) {
    if (head.x >= 1000 || head.x < 0 || head.y >= 1000 || head.y < 0) {
      this.state.start('Game_Over');
    }
  },
...
var Game_Over = {
  create: function () {
    this.game.add.text(420, 230, 'FINAL SCORE:', {font: 'bold 16px sans-serif', fill: '#46c0f9', align: 'center'});
    this.game.add.text(540, 230, score.toString(), {font: 'bold 16px sans-serif', fill: '#fff', align: 'center'});
  }
};
